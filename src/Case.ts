...
class Case extends Utilities {
  ...
  parseTitle({
    ...
    title = Object.values(this.caseParseKeys).filter((key): any => (title?.toLowerCase()?.match(key + "")?.length ?? false))[0]
  ...
  parseCase({
    ...
    const caseMapFiltered: cheerio.Cheerio = caseMap.filter((index: number, caseElement: cheerio.Element) => {
      const { title } = getTitle({
        element: caseElement
      })
      return typeof title != 'undefined';
    });
    let caseData: any = caseMapFiltered.map((index: number, caseElement: cheerio.Element) => {
      const { title } = getTitle({
        element: caseElement
      });
